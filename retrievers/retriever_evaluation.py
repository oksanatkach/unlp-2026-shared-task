...
if __name__ == "__main__":
    ...
    # from retrievers.bm25_retriever import BM25ChunkRetriever
    from retrievers.embedding_retriever import EmbeddingRetriever
    from retrievers.hybrid_retriever import HybridRetriever
    from utils import index_docs_and_save_retriever

    # index_docs_and_save_retriever(BM25ChunkRetriever, config.chunks_800_path, config.bm25_800_retriever_path)
    # index_docs_and_save_retriever(EmbeddingRetriever, config.chunks_800_path, config.embedding_800_retriever_path)
    # index_docs_and_save_retriever(HybridRetriever, config.chunks_800_path, config.hybrid_800_retriever_path)

    # index_docs_and_save_retriever(BM25ChunkRetriever, config.chunks_1000_path, config.bm25_1000_retriever_path)
    # index_docs_and_save_retriever(EmbeddingRetriever, config.chunks_1000_path, config.embedding_1000_retriever_path)
    # index_docs_and_save_retriever(HybridRetriever, config.chunks_1000_path, config.hybrid_1000_retriever_path)

    index_docs_and_save_retriever(EmbeddingRetriever,
                                  config.chunks_800_path,
                                  config.embedding_800_e5large_retriever_path,
                                  embedding_model=config.embedding_model_large
                                  )

    index_docs_and_save_retriever(HybridRetriever,
                                  config.chunks_800_path,
                                  config.hybrid_800_e5large_retriever_path,
                                  embedding_model=config.embedding_model_large
                                  )

    # index_docs_and_save_retriever(EmbeddingRetriever,
    #                               config.chunks_1000_path,
    #                               config.embedding_1000_e5large_retriever_path,
    #                               embedding_model=config.embedding_model_large
    #                               )

    # index_docs_and_save_retriever(HybridRetriever,
    #                               config.chunks_1000_path,
    #                               config.hybrid_1000_e5large_retriever_path,
    #                               embedding_model=config.embedding_model_large
    #                               )

    # index_docs_and_save_retriever(EmbeddingRetriever,
    #                               config.chunks_800_path,
    #                               config.embedding_800_bgem3_retriever_path,
    #                               embedding_model=config.embedding_model_bge
    #                               )

    # index_docs_and_save_retriever(HybridRetriever,
    #                               config.chunks_800_path,
    #                               config.hybrid_800_bgem3_retriever_path,
    #                               embedding_model=config.embedding_model_bge
    #                               )

    # index_docs_and_save_retriever(EmbeddingRetriever,
    #                               config.chunks_1000_path,
    #                               config.embedding_1000_bgem3_retriever_path,
    #                               embedding_model=config.embedding_model_bge
    #                               )

    # index_docs_and_save_retriever(HybridRetriever,
    #                               config.chunks_1000_path,
    #                               config.hybrid_1000_bgem3_retriever_path,
    #                               embedding_model=config.embedding_model_bge
    #                               )
    ###############################################################################################################

    # dev_questions = list(csv.DictReader(open(config.dev_questions_path)))

    # # top_k = 5
    # top_k = 10
    # print(f'Top K {top_k}')
    # print('Chunks max length 800')
    # print('BM25 retriever')
    # retriever = BM25ChunkRetriever()
    # retriever.load(config.bm25_800_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
    #
    # print('Embedding retriever')
    # retriever = EmbeddingRetriever(
    #     embedding_model=config.embedding_model_base,
    #     use_e5_prefix=True,
    #     device=None
    # )
    # retriever.load(config.embedding_800_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
    #
    ...
    #     device=None
    # )
    # retriever.load(config.hybrid_800_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
    ...
    # top_k = 10
    # print(f'Top K {top_k}')
    # print('Chunks max length 1000')
    # print('BM25 retriever')
    # retriever = BM25ChunkRetriever()
    # retriever.load(config.bm25_1000_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
    #
    # print('Embedding retriever')
    # retriever = EmbeddingRetriever(
    ...
    #     device=None
    # )
    # retriever.load(config.embedding_1000_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
    #
    # print('Hybrid retriever')
    # retriever = HybridRetriever(
    ...
    #     device=None
    # )
    # retriever.load(config.hybrid_1000_retriever_path)
    # universal_evaluator(dev_questions, retriever, top_k=top_k)
